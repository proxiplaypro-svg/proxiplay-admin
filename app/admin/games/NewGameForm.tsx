...
type FormState = {
  ...
  prizeUsageDeadlineEnabled: boolean;
  prizeUsageDeadline: string;
};
...
const emptyForm: FormState = {
  ...
  prizeUsageDeadlineEnabled: false,
  prizeUsageDeadline: "",
};
...
function validateImageFile(file: File): string | null {
  ...
  if (file.size > 2 * 1024 * 1024) {
    return "L'image dépasse 2 Mo. Choisis un fichier plus léger.";
  }
  return null;
}
...
export default function NewGameForm({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!merchant) throw new Error("Choisis un commerçant.");
      ...
      if (form.prizeUsageDeadlineEnabled) {
        ...
        const gameEnd = dateAtEndOfDay(form.endDate);
        const usageDeadline = dateAtEndOfDay(form.prizeUsageDeadline);
        if (Number.isNaN(gameEnd.getTime()) || usageDeadline.getTime() < gameEnd.getTime()) {
          throw new Error("La date limite d'utilisation doit être postérieure ou égale à la fin du jeu.");
        }
      }
      ...
      if (form.prizeUsageDeadlineEnabled && form.prizeUsageDeadline) {
        await updateDoc(doc(db, gameCollection, result.game.id), {
          prize_usage_deadline: Timestamp.fromDate(dateAtEndOfDay(form.prizeUsageDeadline)),
        });
      }
    ...
    <section className="min-h-full bg-[#F7F7F5]">
      <div className="mx-auto grid max-w-[900px] gap-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h1 className="text-[22px] font-medium tracking-[-0.02em] text-[#1a1a1a]">{heading}</h1>
            <p className="mt-1 text-[14px] text-[#666]">{intro}</p>
          </div>
          <Link href="/admin/games" className="inline-flex items-center justify-center rounded-[10px] border border-[#E8E8E4] bg-white px-5 py-3 text-[14px] font-medium text-[#666] hover:bg-[#F7F7F5]">
            ← Retour aux jeux
          </Link>
        </div>
        ...
        {createdGame ? (
          <div className="rounded-[12px] border border-[#D8E8C4] bg-[#EAF3DE] p-5">
            ...
            <div className="mt-3 flex flex-wrap gap-3">
              ...
              <Link href={`/admin/games?merchantId=${createdGame.merchantId}`} className="inline-flex items-center justify-center rounded-[10px] border border-[#E0E0DA] bg-white px-4 py-2 text-[13px] font-medium text-[#1A1A1A] hover:bg-[#FAFAF8]">Voir dans Jeux & campagnes</Link>
              <button type="button" onClick={() => setCreatedGame(null)} className="inline-flex items-center justify-center rounded-[10px] border border-[#E8E8E4] bg-white px-4 py-2 text-[13px] font-medium text-[#666] hover:bg-[#F7F7F5]">Créer un autre jeu</button>
            </div>
          </div>
        ) : null}
        ...
        <form className="rounded-[12px] border border-[#E8E8E4] bg-white p-5" onSubmit={(event) => { event.preventDefault(); void handleSave(); }}>
          <div className="grid gap-4 sm:grid-cols-2">
            <label className="flex flex-col gap-1.5 sm:col-span-2">
              ...
              <select className="rounded-[8px] border border-[#E0E0DA] px-3 py-2 text-[14px] text-[#1a1a1a] focus:outline-none focus:ring-2 focus:ring-[#639922]" value={form.merchantId} onChange={(event) => setForm((prev) => ({ ...prev, merchantId: event.target.value }))} disabled={loadingMerchants} required>
                <option value="">{loadingMerchants ? "Chargement..." : "Choisir un commerçant"}</option>
            ...
            <label className="flex flex-col gap-1.5">
              ...
              <input className="rounded-[8px] border border-[#E0E0DA] px-3 py-2 text-[14px] text-[#1a1a1a] focus:outline-none focus:ring-2 focus:ring-[#639922]" type="date" value={form.startDate} onChange={(event) => setForm((prev) => ({ ...prev, startDate: event.target.value }))} required />
            </label>
            <label className="flex flex-col gap-1.5">
              <span className="text-[12px] font-medium text-[#666]">Date de fin</span>
              <input className="rounded-[8px] border border-[#E0E0DA] px-3 py-2 text-[14px] text-[#1a1a1a] focus:outline-none focus:ring-2 focus:ring-[#639922]" type="date" value={form.endDate} onChange={(event) => setForm((prev) => ({ ...prev, endDate: event.target.value, prizeUsageDeadline: prev.prizeUsageDeadline && prev.prizeUsageDeadline < event.target.value ? event.target.value : prev.prizeUsageDeadline }))} required />
            </label>

            <div className="sm:col-span-2 rounded-[10px] border border-[#E8E8E4] bg-[#FAFAF8] p-4">
              <div className="flex items-center justify-between gap-4">
                <div>
                  <p className="text-[13px] font-medium text-[#1A1A1A]">Date limite d’utilisation du lot</p>
                  <p className="mt-1 text-[12px] text-[#777]">Optionnel : informe le gagnant de la date avant laquelle retirer ou utiliser son lot.</p>
                </div>
                <input type="checkbox" checked={form.prizeUsageDeadlineEnabled} onChange={(event) => setForm((prev) => ({ ...prev, prizeUsageDeadlineEnabled: event.target.checked, prizeUsageDeadline: event.target.checked ? (prev.prizeUsageDeadline || prev.endDate) : "" }))} className="h-4 w-4 rounded border-[#D7D7D2]" />
              </div>
              {form.prizeUsageDeadlineEnabled ? (
                <label className="mt-3 flex flex-col gap-1.5">
                  <span className="text-[12px] font-medium text-[#666]">Date limite d’utilisation</span>
                  <input className="rounded-[8px] border border-[#E0E0DA] bg-white px-3 py-2 text-[14px] text-[#1a1a1a] focus:outline-none focus:ring-2 focus:ring-[#639922]" type="date" min={form.endDate || undefined} value={form.prizeUsageDeadline} onChange={(event) => setForm((prev) => ({ ...prev, prizeUsageDeadline: event.target.value }))} required />
                </label>
              ) : null}
            </div>
            ...
            <label className="flex flex-col gap-1.5 sm:col-span-2">
              ...
              <div className="flex items-center gap-3">
                {imagePreviewUrl ? <img src={imagePreviewUrl} alt="" className="h-12 w-12 flex-none rounded-[8px] object-cover" /> : null}
                <input ref={imageInputRef} type="file" accept="image/*" onChange={(event) => { const file = event.target.files?.[0]; if (!file) { setImageFile(null); return; } const validationError = validateImageFile(file); if (validationError) { setError(validationError); event.target.value = ""; setImageFile(null); return; } setError(null); setImageFile(file); }} className="text-[13px] text-[#666] file:mr-3 file:rounded-[8px] file:border file:border-[#E0E0DA] file:bg-white file:px-3 file:py-1.5 file:text-[12px] file:font-medium file:text-[#666] hover:file:bg-[#F7F7F5]" />
              </div>
            </label>
          </div>
          ...
          <div className="mt-5 rounded-[10px] border border-[#E8E8E4] bg-[#FAFAF8] p-4">
            <div className="flex items-center justify-between">
              ...
              <button type="button" onClick={() => setSecondaryPrizes((current) => [...current, createEmptySecondaryPrize()])} className="rounded-[8px] border border-[#E0E0DA] bg-white px-3 py-1.5 text-[12px] font-medium text-[#1A1A1A] hover:bg-[#F7F7F5]">+ Ajouter un lot secondaire</button>
            </div>
                ...
                {secondaryPrizes.map((prize, index) => (
                  <div key={prize.id} className="rounded-[8px] border border-[#E8E8E4] bg-white p-3">
                    ...
                    <div className="mt-2 grid gap-2 sm:grid-cols-[2fr_1fr]">
                      ...
                      <input className="rounded-[8px] border border-[#E0E0DA] px-3 py-2 text-[13px] text-[#1a1a1a] focus:outline-none focus:ring-2 focus:ring-[#639922]" type="number" min="0" step="1" placeholder="Quantité" value={prize.count} onChange={(event) => updateSecondaryPrize(prize.id, (current) => ({ ...current, count: event.target.value }))} />
                    </div>
                    <textarea className="mt-2 w-full resize-y rounded-[8px] border border-[#E0E0DA] px-3 py-2 text-[13px] text-[#1a1a1a] focus:outline-none focus:ring-2 focus:ring-[#639922]" rows={2} placeholder="Description (facultatif)" value={prize.description} onChange={(event) => updateSecondaryPrize(prize.id, (current) => ({ ...current, description: event.target.value }))} />
                  </div>
                ))}
              </div>
            ) : null}
