"use client";
import type { ProspectFields as Fields } from "@/lib/prospection/model";
...
export function ProspectFields({ value, onChange }: { value: Partial<Fields>; onChange: (value: Partial<Fields>) => void }) {
  ...
  return <><section className={s.panel}><h2>Entreprise</h2><div className={s.grid}>{company.map(field)}</div></section><section className={s.panel}><h2>Contact</h2><div className={s.grid}>{contact.map(field)}</div></section></>;
}
